...
function buildAudioFilePath(zeroBasedIndex) {
  ...
  const subfolder      = oneBasedNumber <= 1000 ? 'p1/' : 'p2/';
  return `${AUDIO_FOLDER}${subfolder}${AUDIO_PREFIX}${paddedNumber}${AUDIO_EXTENSION}`;
}
...
function loadDescriptionForIndex(zeroBasedIndex, callbackFn) {
  try {
    ...
    const paddedNumber   = String(oneBasedNumber).padStart(4, '0');
    const subfolder      = oneBasedNumber <= 1000 ? 'p1/' : 'p2/';
    const scriptPath     = `./data/text/${subfolder}desc-${paddedNumber}.js`;
